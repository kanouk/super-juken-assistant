        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {showHistoryButton && onShowHistory && (
            <Button
              ...
              className="gap-2 hidden sm:flex hover:bg-gray-50 border-gray-300 transition-colors shadow-sm"
            >
          ...
          {showNewChatButton && onNewChat && (
            <Button
              ...
              size="sm"
              onClick={onNewChat}
              className="gap-2 hover:bg-blue-50 border-blue-300 text-blue-700 transition-colors shadow-sm"
            >
              <Plus className="h-4 w-4" />
              <span className="hidden sm:inline">新規チャット</span>
            </Button>
          )}
          <div className="text-right hidden lg:block bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">
            <div className="flex items-center gap-1.5 text-sm text-gray-600 font-medium">
              <Cpu className="h-3 w-3" />
              使用モデル
            </div>
            <p className="text-xs text-gray-500 font-mono">{currentModel}</p>
          </div>
        </div>
      </div>
    </div>
